...
for path in sorted(glob.glob("/tmp/bake/s_*.glb.gz")):
...
for tag in ("mustache", "car", "turtle"):
    im = Image.open(f"/tmp/bake/{tag}.png").convert("RGB"); im.thumbnail((512, 512)); im.save(f"/tmp/bake/thumb_{tag}.jpg", "JPEG", quality=82)
    print(f"thumb_{tag}.jpg {os.path.getsize(f'/tmp/bake/thumb_{tag}.jpg')/1e3:.0f} KB {im.size}")
